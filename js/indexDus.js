...
secret.addEventListener('click', () => {
    ...
    toggleButton.addEventListener('click', () => {
        ...
        if (!isToggled) {
            if (dusResponse !== undefined) { writeWeather(dusResponse); }
            else { getWeather(); }
...
const writeWeather = function(data) {
    ...
    locationAndTemp = 'Düsseldorf | ' + Math.round(data.list[0].main.temp) + '&deg;';
...
const getWeather = function() {
    //weather call
    fetch('https://api.openweathermap.org/data/2.5/forecast?lat=51.22172&lon=6.77616&exclude=minutely,hourly,daily,alerts&appid=80daf6978b24a949df62669da4146061&units=metric')
    .then((response) => response.json())
    .then((data) => {
        dusResponse = data;
        console.log(dusResponse);
        writeWeather(dusResponse); });
    //moon call
    let utc = new Date().toJSON().slice(0,10).replace(/-/g,'-');
    fetch('https://api.weatherapi.com/v1/astronomy.json?key=88d21e164d0d49d99a182132231304&q=Dusseldorf&dt=' + utc)
    .then((response) => response.json())
    .then((data) => {
        moonResponse = data;});
    }
